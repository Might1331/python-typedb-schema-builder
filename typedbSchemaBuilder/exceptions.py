        ...
        for i in range(0, n):
            ...
            if query[0] == "key" or query[0] == "unique":
                ...
                if owns not in self.types[type_].attributes:
                    message= "Error: Type = " + type_ + " does not own:" + owns + "\nqid:" + str(query[-1])
        ...
        for i in range(0, n):
            ...
            if query[0] in ["abstract","value","regex"]:
                if query[1] not in self.types.keys():
                    message="Error: type = "+query[1]+" is not defined"+"\nqid: "+str(query[-1])
                    raise Exception(message)
                ...
                if query[1] not in self.types.keys():
                    ...
                    message="Error:"+query[1]+" is not an relation type"+"\nqid: "+str(query[-1])
                    raise Exception(message)
                ...
                if query[1] not in self.types.keys():
                    ...
                    message="Error:"+query[1]+" is not an entity type"+"\nqid: "+str(query[-1])
                    raise Exception(message)
                
                if query[-2] not in self.types.keys():
                    message="Error:"+query[-2]+" is not defined"+"\nqid: "+str(query[-1])
                    raise Exception(message)
                elif "attribute" != self.types[query[-2]].root_type:
                    message="Error:"+query[-2]+" is not an attribute type"+"\nqid: "+str(query[-1])
                    raise Exception(message)
            elif query[0] in ["sub"]:
                if query[2] not in self.types.keys():
                    message="Error: type = "+query[2]+" is not defined"+"\nqid: "+str(query[-1])
                    raise Exception(message)
                ...
                if "entity" != self.types[query[1]].root_type:
                    message="Error: not entity, type = "+query[1]+"\nqid: "+str(query[-1])
                    raise Exception(message)
                
                if query[2] not in self.types.keys():
                    message="Error: relationship type = "+query[2]+" is not defined"+"\nqid: "+str(query[-1])
                    raise Exception(message)
                if "relation" != self.types[query[2]].root_type:
                    message="Error: not relationship, type = "+query[2]+"\nqid: "+str(query[-1])
                    raise Exception(message)
                
                if query[-2] not in self.types[query[2]].relation_roles:
                    message="Error: relationship type = "+query[2]+" has no role = "+query[-2]+"\nqid: "+str(query[-1])
                    raise Exception(message)
